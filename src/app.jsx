...
function App() {
  const ADMIN_EMAIL = "bikomusic3@gmail.com"; 
  const ADMIN_PASSWORD = "Aass1122@@"; 
  ...
  const [tracksData, setTracksData] = useState([
    { id: 1, title: "Mohamed Biko - Track One (Demo)", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3" },
    { id: 2, title: "Mohamed Biko - Cyber Hustle (Demo)", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3" },
    { id: 3, title: "Mohamed Biko - Street Vibe (Demo)", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3" },
    { id: 4, title: "Mohamed Biko - Phantom Beats", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3" }
  ]);

  const [videosData, setVideosData] = useState([
    { id: 1, title: "Mohamed Biko - Official Music Video", youtubeId: "dQw4w9WgXcQ", thumbnail: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=500&q=80" },
    { id: 2, title: "Biko Live Concert 2026", youtubeId: "3v8v9xK7X_g", thumbnail: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=500&q=80" }
  ]);
    ...
    const handleMouseOver = (e) => {
      ...
      try {
        ...
        if (
          ...
          (targetElement.closest && (targetElement.closest('.track-card-horizontal') || targetElement.closest('.video-card') || targetElement.closest('.hero-social-icon')))
        ) {
          setIsHovered(true);
        } else {
          setIsHovered(false);
  ...
  const handleLoginSubmit = (e) => {
    ...
    if (emailInput === ADMIN_EMAIL && passwordInput === ADMIN_PASSWORD) {
      ...
      setLoginError('عفواً.. الحساب أو كلمة المرور غير مطابقة!');
    }
  };
  ...
  const handleAddTrack = (e) => {
    ...
    if (tracksData.length === 0) setCurrentTrack(newTrack);
    setNewTrackTitle(''); setNewTrackUrl('');
  };

  const handleDeleteTrack = (id) => {
    const filtered = tracksData.filter(t => t.id !== id);
    setTracksData(filtered);
    if (currentTrack && currentTrack.id === id) {
      if (isPlaying) audioRef.current.pause();
      setIsPlaying(false);
      setCurrentTrack(filtered[0] || null);
    }
  };
  ...
  const handleAddVideo = (e) => {
    ...
    const newVideo = { id: Date.now(), title: newVideoTitle, youtubeId: newVideoId, thumbnail: `https://img.youtube.com/vi/${newVideoId}/hqdefault.jpg` };
    setVideosData([...videosData, newVideo]);
    setNewVideoTitle(''); setNewVideoId('');
  };
      ...
      <div className="admin-dashboard-layout" style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', borderBottom: '1px solid #27272a', paddingBottom: '20px' }}>
          <div>
            ...
            <p style={{ color: '#71717a', marginTop: '5px' }}>مرحباً بك يا فنان - يمكنك التحكم في المحتوى الحي الآن</p>
          </div>
          <button className="btn-dark" style={{ borderColor: '#ff3366', color: '#ff3366' }} onClick={() => { setEmailInput(''); setPasswordInput(''); window.history.pushState({}, '', '/'); setView('site'); }}>تسجيل الخروج 🚪</button>
        </div>
          ...
          <div style={{ background: '#111113', padding: '30px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {tracksData.map((track, i) => (
                <div key={track.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#18181b', padding: '12px 15px', borderRadius: '6px' }}>
                  <span style={{ fontWeight: '600', fontSize: '14px' }}>{i+1}. {track.title}</span>
                  <button onClick={() => handleDeleteTrack(track.id)} style={{ background: '#ff3366', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>حذف</button>
                </div>
              ))}
            </div>
          </div>
          <div style={{ background: '#111113', padding: '30px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {videosData.map((video, i) => (
                <div key={video.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#18181b', padding: '12px 15px', borderRadius: '6px' }}>
                  <span style={{ fontWeight: '600', fontSize: '14px' }}>{i+1}. {video.title}</span>
                  <button onClick={() => setVideosData(videosData.filter(v => v.id !== video.id))} style={{ background: '#ff3366', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>حذف</button>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', width: '100%', overflowX: 'hidden' }} className={isHovered ? 'cursor-hover-active' : ''}>
      ...
      <nav className="navbar">
        <div className="logo" style={{ fontSize: '22px', fontWeight: '950', letterSpacing: '2px' }}>
          M.BIKO<span className="text-neon">OFFICIAL</span>
        </div>
      ...
      <header className="hero-container" id="home">
        <div className="hero-grid">
          <div className="hero-content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', textAlign: 'left' }}>
            <span style={{ fontSize: '12px', fontWeight: '900', letterSpacing: '4px', color: '#00ffcc', marginBottom: '10px' }}>
              OFFICIAL ARTIST
            </span>
            ...
            <p dir="rtl" style={{ color: '#a1a1aa', fontSize: '18px', lineHeight: '1.8', marginBottom: '35px', maxWidth: '540px', textAlign: 'left' }}>
              بصمة صوتية حادة تشرح <span className="text-neon" style={{ fontWeight: '700' }}>واقع الشارع</span> بدون تزييف. 
              <br />
              إيقاعات ثقيلة، كلمات تضرب في العصب، وهندسة صوتية تعيد رسم مشهد الـ <span style={{ color: '#ffffff', fontWeight: '700' }}>Underground</span> العربي.
            </p>
            ...
            <div className="social-icons-wrap" style={{ display: 'flex', gap: '15px', alignItems: 'center', marginTop: '5px' }}>
              <a href="https://www.youtube.com/@mohamedbiko11" target="_blank" rel="noreferrer" className="hero-social-icon yt-icon" title="YouTube">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
              </a>
              <a href="https://www.instagram.com/mohamed_biko1" target="_blank" rel="noreferrer" className="hero-social-icon ig-icon" title="Instagram">
        ...
        <div className="modern-tracks-layout" style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
          ...
          <div className="main-player-card scroll-reveal" style={{ maxWidth: '800px', margin: '0 auto', width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '30px', flexWrap: 'wrap', justifyContent: 'center' }}>
              <div className={`player-cover-art ${isPlaying ? 'spinning' : ''}`} style={{ margin: '0' }}>💿</div>
          ...
          <div className="tracks-horizontal-container" style={{ display: 'flex', gap: '20px', overflowX: 'auto', padding: '10px 5px 20px 5px', scrollbarWidth: 'thin' }}>
            {tracksData.map((track, index) => (
              <div key={track.id} className={`track-card-horizontal ${currentTrack && currentTrack.id === track.id ? 'active' : ''}`} onClick={() => { setCurrentTrack(track); setIsPlaying(true); }}>
                <div className="track-card-top">
                  <span className="track-number-badge">{index + 1 < 10 ? `0${index + 1}` : index + 1}</span>
                  <div className="pulse-wave-icon">🎵</div>
                </div>
                <div className="track-card-body">
                  <span className="track-card-title-text">{track.title}</span>
                </div>
      ...
      <section className="videos-section scroll-reveal" id="videos">
        ...
        <div className="videos-grid">
          {videosData.map((video) => (
            <div key={video.id} className="video-card scroll-reveal" onClick={() => {
      ...
      <footer className="main-footer">
        <div className="footer-content">
          ...
          <div className="footer-socials">
            <a href='https://www.youtube.com/@mohamedbiko11' target="_blank" rel="noreferrer" className="social-link yt">YOUTUBE</a>
            <a href='https://www.instagram.com/mohamed_biko1' target="_blank" rel="noreferrer" className="social-link ig">INSTAGRAM</a>
          </div>
        </div>
      </footer>
        ...
        <div className="lightbox-overlay" onClick={() => setActiveVideoId(null)}>
          <div className="lightbox-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="video-responsive-container">
              <iframe src={`https://www.youtube.com/embed/${activeVideoId}?autoplay=1`} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
            </div>
          </div>
        </div>
      )}
